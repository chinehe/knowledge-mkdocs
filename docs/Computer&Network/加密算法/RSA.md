...
def generate_key_pair(bits: int = 2048) -> tuple:
    """生成 RSA 密钥对"""
...
if __name__ == "__main__":
    ...
    print(f"密钥长度: {private_key.size_in_bits()} 位")

    # 加密/解密
    plaintext = b"Hello, RSA-OAEP Encryption!"
    print(f"\n=== 加密/解密 ===")
    ...
    # 导出密钥
    print(f"\n=== 密钥导出 ===")
    print(private_key.export_key().decode()[:100] + "...")
    print(public_key.export_key().decode()[:100] + "...")
```
...
public class RSAExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("密钥长度: 2048 位");

        // 加密/解密
        byte[] plaintext = "Hello, RSA-OAEP Encryption!".getBytes();
        System.out.println("\n=== 加密/解密 ===");
        ...
        byte[] decrypted = decrypt(keyPair.getPrivate(), ciphertext);
        System.out.println("解密: " + new String(decrypted));

        // 签名/验签
        byte[] message = "This message needs to be signed.".getBytes();
        System.out.println("\n=== 签名/验签 ===");
        ...
        boolean isValid = verify(keyPair.getPublic(), message, signature);
